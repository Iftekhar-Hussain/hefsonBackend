...
const getLocationAsync = (lat, lng, timestamp) => {
  return new Promise((resolve, reject) => {
    getOffsetFromAddress(lat, lng, timestamp, (location) => {
      if (location) {
        resolve(location);
      } else {
...
const calculateTimeInUTC = async (date, time, latitude, longitude) => {
  ...
  let actualTime = dateConvertTimeStamp(date, time);
  var lat = latitude,
    lng = longitude;
  var timestamp1 = parseInt(moment.utc(actualTime).valueOf() / 1000);
  const fakelocation = await getLocationAsync(lat, lng, timestamp1);

  // get actual time
  let gg = convertToActualDate(date, time, fakelocation.timeZoneId);
  actualTime = parseInt(moment.utc(gg).valueOf() / 1000);
  return {
    time: actualTime,
...
export const addShipment = async (payload) => {
  let trailerExist = await SHIPMENTMODEL.findOne({
    trailerId: payload.trailerId,
    isCompleted: false,
    isCancelled: false,
    //  $or: [{ isCompleted: false , isCancelled:true}],
  });
  if (trailerExist) {
    return { status: 400, message: "trailer" };
  }

  let truckExist = await SHIPMENTMODEL.findOne({
    truckId: payload.trailerId,
    $or: [{ isCompleted: false }, { isDeleted: false }, { isCancelled: false }],
  });
  if (truckExist) {
    return { status: 400, message: "truck" };
  }

  let trailerData = await TRAILERMODEL.findOne({
    _id: mongoose.Types.ObjectId(payload.trailerId),
  });
  let sensorData = await SENSORMODEL.findOne({
    _id: mongoose.Types.ObjectId(trailerData.sensorId),
  });
  let startTime = await calculateTimeInUTC(
    payload.shipper[0].pickupDate,
    payload.shipper[0].pickupTime,
    payload.shipper[0].latitude,
    payload.shipper[0].longitude
  );
  payload.startTime = startTime.time;
  payload.startOffset = startTime.offset;

  let endTime = await calculateTimeInUTC(
    payload.receiver[0].deliveryDate,
    payload.receiver[0].deliveryTime,
    payload.receiver[0].latitude,
    payload.receiver[0].longitude
  );
  ...
  for (let i = 0; i < payload.shipper.length; i++) {
    let shipperTimeCal = await calculateTimeInUTC(
      payload.shipper[i].pickupDate,
      payload.shipper[i].pickupTime,
      payload.shipper[i].latitude,
      payload.shipper[i].longitude
    );
  ...
  for (let j = 0; j < payload.shipper.length; j++) {
    let receiverTimeCal = await calculateTimeInUTC(
      payload.receiver[j].deliveryDate,
      payload.receiver[j].deliveryTime,
      payload.receiver[j].latitude,
      payload.receiver[j].longitude
    );
  ...
  payload.totalDistance = await getDistance(
    payload.shipper[0].latitude,
    payload.shipper[0].longitude,
    payload.receiver[0].latitude,
    payload.receiver[0].longitude
  ...
  let dt2 =
    payload.receiver[0].deliveryDate + " " + payload.receiver[0].deliveryTime;
  payload.totalHours = diff_hours(new Date(dt1), new Date(dt2));

  await USERMODEL.findOneAndUpdate(
    { _id: mongoose.Types.ObjectId(payload.carrierId) },
    { $set: userObj }
  );

  let trailerInfo = await TRAILERMODEL.findOne({
    _id: mongoose.Types.ObjectId(payload.trailerId),
  });
  let currentHours =
    parseInt(trailerInfo.currentHours) + parseInt(payload.totalHours);
    let totalTrailerHours =
    parseInt(trailerInfo.engineHours) + parseInt(payload.totalHours);
  ...
  let saveData = await SHIPMENTMODEL.saveShipment(payload);
  let sensorToken = "";
  ...
  let options = await axios({
    ...
    data: {
      ...
      FAssetGUID: sensorData.FAssetGUID, //"fbfc256e-7225-4b97-b297-edf55ccecb15", //shipmentIds,
      FInsType: "BASE48",
      FOffline: "1",
      FT_BASE48Ins: {
        FOperationType: 3,
        FAssetID: sensorData.FAssetID, //"835210000201",
        FHighTempAlarm: 1,
        FLowTempAlarm: 1,
 ...
 */
export const listShipment = async (user, payload) => {
  let query = {
    isDeleted: false,
  };
  ...
  if (
    ...
    query = { ...query, isCompleted: false, isCancelled: false };
  }

  if (payload.status == "complete") {
    query = { ...query, isCompleted: true };
  }
  ...
  if (user.role != 1) {
    let userData = await USERMODEL.findOne({ role: 1 });
    sensorToken = userData.sensorToken;
    query = { ...query, carrierId: mongoose.Types.ObjectId(user.userId) };
  } else {
  ...
  if (payload["search"] && payload["search"] != "") {
    ...
    query = {
      ...
      $or: [
        ...
        { trailerNumber: { $regex: regex } },
        { driverName: { $regex: regex } },
        { broker: { $regex: regex } },
        { brokerAgent: { $regex: regex } },
      ],
    };
  }

  var data = await SHIPMENTMODEL.listShipment(
    user,
    payload,
    query,
    sortBy,
  ...
  finalData = await Promise.all(
    finalData.map(async (k, index) => {
      ...
      if (obj.isStart == true) {
        obj["coveredHours"] = await getDistance(
 ...
 */
export const detailShipment = async (user, payload) => {
  let shipmaentId = payload.id;
  let query = {
    _id: mongoose.Types.ObjectId(shipmaentId),
    isActive: true,
    isDeleted: false,
  };

  var data = await SHIPMENTMODEL.detailShipment(payload, query);
  let sensorToken = "";
  //if (user.role != 1) {
  let userData = await USERMODEL.findOne({ role: 1 });
  sensorToken = userData.sensorToken;
  ...
  if (data[0].isStart == true) {
    data[0].coveredHours = await getDistance(
      data[0].shipper[0].latitude,
      data[0].shipper[0].longitude,
  ...
  data[0].leftDistancePercentage = (
    ((data[0].totalDistance - data[0].coveredHours) / data[0].totalDistance) *
    100
  ).toFixed(2);
  ...
  let endTime = await calculateTimeInUTC(
    data[0].receiver[0].deliveryDate,
    data[0].receiver[0].deliveryTime,
    data[0].receiver[0].latitude,
    data[0].receiver[0].longitude
  );
  ...
  let shipperTimeCal = await calculateTimeInUTC(
    data[0].shipper[0].pickupDate,
    data[0].shipper[0].pickupTime,
    data[0].shipper[0].latitude,
    data[0].shipper[0].longitude
  );

  console.log("shipperTimeCal.utcDate", shipperTimeCal.utcDate);
  console.log("shipperTimeCal.endTime", endTime.utcDate);
...
export const completeShipmentDetail = async (user, payload) => {
  let shipmaentId = payload.id;
  let query = {
    _id: mongoose.Types.ObjectId(shipmaentId),
    isActive: true,
...
export const updateShipment = async (payload) => {
  ...
  let trailerExist = await SHIPMENTMODEL.findOne({
    ...
    trailerId: payload.trailerId,
    $and: [{ isCompleted: false }, { isCancelled: false }],
  });
...
export const updateStatus = async (payload) => {
  ...
  if (payload.status == "complete") {
    ...
    setData = { ...setData, isCompleted: true };
 ...
 */
export const deleteShipment = async (payload) => {
  return await SHIPMENTMODEL.deleteShipment(payload);
};
 ...
 */
export const listActiveShipment = async (payload) => {
  let query = {
    isDeleted: { $eq: false },
  ...
  if (payload["search"] && payload["search"] != "") {
    ...
    var data = await SHIPMENTMODEL.listActiveShipment(
      payload,
      query,
      sortBy,
      sortValue
 ...
 */
export const listInactiveShipment = async (payload) => {
  let query = {
    isDeleted: { $eq: false },
    isCompleted: { $eq: true },
  };
 ...
 * @description - list of shipment
 */
export const listAlarm = async (user, payload) => {
  ...
  if (payload.status == "past") {
    match = { ...match, createdAt: { $lte: startOfDay } };
  }
  ...
  let count = await ALARMMODEL.aggregate(query);
  let total = count.length;
  query.push({ $skip: skip });
  query.push({ $limit: limit });
  let alarmList = await ALARMMODEL.aggregate(query);
  return {
    data: alarmList,
    total: total,
  };
  ...
  return new Promise(async (resolve) => {
    await setTimeout(async () => {
      let historyData = await HISTORYMODEL.find(
        {
          shipmentId: mongoose.Types.ObjectId(payload.id),
        },
        {
          _id: 0,
          createdAt: 0,
...
export const shareLink = async (user, payload) => {
