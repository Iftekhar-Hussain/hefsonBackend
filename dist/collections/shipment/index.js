...
class ShipmentClass {
  ...
  static listShipment(user, payload, match, sortBy, sortValue) {
    ...
    let skip = JSON.parse((payload.pageNum - 1) * limit); // const pagination = [
  ...
  static listActiveShipment(payload, match, sortBy, sortValue) {
    const query = [{
      ...
      $group: {
        _id: "$shipper.loadId",
        pickupName: {
          $first: "$shipper.pickupName"
        },
        pickupAddress: {
          $first: "$shipper.pickupAddress"
        },
        pickupDate: {
          $first: "$shipper.pickupDate"
        },
        pickupTime: {
          $first: "$shipper.pickupTime"
        },
        poNumber: {
          $first: "$shipper.poNumber"
        },
        receiver: {
          $first: "$receiver"
        },
        ...
        modelYearTruck: {
          $first: "$modelYearTruck"
        },
        truckColor: {
          $first: "$truckColor"
        },
        issuedStateTruck: {
          $first: "$issuedStateTruck"
        },
        registrationExpDateTruck: {
          $first: "$registrationExpDateTruck"
        },
        numberPlateTruck: {
          $first: "$numberPlateTruck"
        },
        trailerNumber: {
          $first: "$trailerNumber"
        },
        numberPlateTrailer: {
          $first: "$numberPlateTrailer"
        },
        modelYearTrailer: {
          $first: "$modelYearTrailer"
        },
        issuedStateTrailer: {
          $first: "$issuedStateTrailer"
        },
        registrationExpDateTrailer: {
          $first: "$registrationExpDateTrailer"
        },
        driverName: {
          $first: "$driverName"
        },
        driverHefsonId: {
          $first: "$driverHefsonId"
        },
        licenseNumber: {
          $first: "$licenseNumber"
        },
...
_dbSchema.default.loadClass(ShipmentClass);

var _default = _mongoose.default.model("Shipments", _dbSchema.default);
